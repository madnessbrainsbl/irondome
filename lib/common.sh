#!/bin/bash
# Sourced by bin/irondome and every lib/*.sh. The paths defined below are
# consumed by those callers, which shellcheck cannot see while linting this
# file on its own — hence the file-level SC2034 exemption.
